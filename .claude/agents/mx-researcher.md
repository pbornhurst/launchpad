...
2. **Pull Master Hub data** — Use the Bash tool: `gws sheets +read --spreadsheet 1ndVs2lPhS5frpkEV0KzK7ec5aS18fmr9h1BQEu099E4 --range "B1:E800" 2>/dev/null` (gws has no 50-row display cap — a single ranged read returns the whole sheet):
...
3. **Pull Volume Drop Data** — Use the Bash tool: `gws sheets +read --spreadsheet 1bu0fWwKWQQeI8nrkhGKA68dTzKRtIh_MXAPeqze0NX0 --range "A1:Z800" 2>/dev/null`:
...
6. **Search emails** — Use `mcp__claude_ai_Gmail__search_threads`:
   - Search for the mx name
   - Summarize the last 5 relevant email threads with dates, subject, and key points

7. **Pull Product Feedback** — Use the Bash tool: `gws sheets +read --spreadsheet 1-EylRCLxhpStfEoj-8ga9Ex_26dHBoWgxU6Yr_hT0Y4 --range "The Final Final Boss" 2>/dev/null`:
   - Filter for this mx's feedback entries

8. **Read Running Notes** — If a notes doc link was found in Master Hub:
   - Use `mcp__claude_ai_Google_Drive__read_file_content` with the notes doc ID
...
1. Build the styled HTML, write it to a temp file, then convert it to a Doc via the Bash tool: `gws drive files create --json '{"name":"[Merchant Name] — Deep Dive Dossier | Store [STORE_ID]","mimeType":"application/vnd.google-apps.document","parents":["1LC-N9ib_c43jJeXkbRm0iO_3FswL-wrn"]}' --upload dossier.html --upload-content-type "text/html" 2>/dev/null` (returns `{"id":...}`; preserves headings + tables)
...
5. After creating the doc, share it with `doordash.com` domain as reader via the Bash tool: `gws drive permissions create --params '{"fileId":"[DOC_ID]"}' --json '{"role":"reader","type":"domain","domain":"doordash.com"}' 2>/dev/null`
